...
class W_Focus extends Widget {
  ...
  W_Focus(PApplet _parent){
    ...
    for (int i = 0; i < serialPortsLocal.length; i++) {
      ...
      if(!tempPort.equals(openBCI_portName)) serialList.add(tempPort);
    }
    ...
    addDropdown("FocusSerialOut", "Serial", serialList, 0);
  ...
  void updateFocusState() {
    ...
    if (alpha_avg > alpha_thresh && alpha_avg < alpha_upper && beta_avg < beta_thresh) {
      if(!isFocused) timeOfLastTrip = millis();
  ...
  void sendFocusSerial() {
    ...
    float timeToWaitThresh = 2000;
    if (wasConnected) {
      try {
        if (isFocused && !wasWritten){
          
          if (!firstWritten && millis() - timeOfLastTrip >= timeToWaitThresh){
            println("DATA SENT");
            w_focus.serialOut.write("2");
            delay(1000);
            w_focus.serialOut.write("50");
            
            firstWritten = true;
          }
          else if (millis() - timeOfLastTrip >= (4*timeToWaitThresh)){
            println("MORE DATA SENT");
            w_focus.serialOut.write("2");
            delay(1000);
            w_focus.serialOut.write("120");
...
void FocusSerialOut(int n){
  
  if(!w_focus.serialList.get(n).equals("NONE")) {
     try{

          w_focus.serialOut = new Serial(w_focus.parent,(String)w_focus.serialList.get(n),9600);
          verbosePrint("Connected to Serial");
          //println(w_focus.serialOut.readString());
          w_focus.wasConnected = true;
          
        }
        catch(Exception e){
          w_focus.wasConnected = false;
          println(e);
          verbosePrint("Could not connect to Serial");
        }
  }
